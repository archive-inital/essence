...
public class MethodClassifier {
	...
	public static List<RankResult<MethodInstance>> rank(MethodInstance src, MethodInstance[] dsts, ClassifierLevel level, OldClassEnvironment env) {
		return rank(src, dsts, level, env, Double.POSITIVE_INFINITY);
	}

	public static List<RankResult<MethodInstance>> rank(MethodInstance src, MethodInstance[] dsts, ClassifierLevel level, OldClassEnvironment env, double maxMismatch) {
	...
	private static AbstractClassifier methodTypeCheck = new AbstractClassifier("method type check") {
		@Override
		public double getScore(MethodInstance methodA, MethodInstance methodB, OldClassEnvironment env) {
	...
	private static AbstractClassifier signature = new AbstractClassifier("signature") {
		@Override
		public double getScore(MethodInstance methodA, MethodInstance methodB, OldClassEnvironment env) {
			Signature.MethodSignature sigA = methodA.getSignature();
			Signature.MethodSignature sigB = methodB.getSignature();
