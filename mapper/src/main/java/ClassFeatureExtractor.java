...
public class ClassFeatureExtractor implements LocalClassEnv {
	public ClassFeatureExtractor(OldClassEnvironment env) {
		this.env = env;
	}
	...
	private ClassInstance readClass(Path path, Predicate<ClassNode> nameObfuscated) {
		ClassNode cn = OldClassEnvironment.readClass(path, false);
	...
	public void process(Pattern nonObfuscatedMemberPattern) {
		...
		for (ClassInstance cls : initialClasses) {
			OldClassEnvironment.processClassA(cls, nonObfuscatedMemberPattern);
		}
	...
	public ClassInstance getCreateClassInstance(String id, boolean createUnknown) {
		...
		if (id.charAt(0) == '[') { // array type
			...
			ClassInstance elementClass = OldClassEnvironment.getArrayCls(this, id);
			...
			if (ret == cls) { // cls was added
				OldClassEnvironment.addSuperClass(ret, "java/lang/Object");
			}
		} else {
	...
	private ClassInstance createClassPathClass(String id) {
		...
		ClassNode cn = OldClassEnvironment.readClass(file, false);
		...
		if (initStep > 0) OldClassEnvironment.processClassA(cls, null);
		if (initStep > 1) processClassB(cls);
		if (initStep > 2) processClassC(cls);
		if (initStep > 3) processClassD(cls, new Analysis.CommonClasses(this));

		return cls;
	}

	@Override
	public OldClassEnvironment getGlobal() {
		return env;
	}

	final OldClassEnvironment env;
